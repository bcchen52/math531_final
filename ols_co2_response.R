# =============================================================================
# Advanced OLS Analysis: CO2 Per Capita Emissions as Response
# =============================================================================
...
#
# Response: co2_per_capita (log-transformed)
# Predictors: GDP, year, energy, and emission sources (selected via stepwise)
# =============================================================================
...
output_dir <- "plots/ols_co2_response"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

cat("\n", strrep("=", 80), "\n")
cat("ADVANCED OLS ANALYSIS: CO2 PER CAPITA EMISSIONS PREDICTION\n")
cat("Response: co2_per_capita (log-transformed)\n")
cat(strrep("=", 80), "\n\n")
...
data1 <- data0 %>%
  ...
  drop_na(co2_per_capita)  # Response variable must be non-NA

# Select variables (co2_per_capita is response, includes component emissions and GDP)
wanted_columns <- c(
  "iso_code",
  "year",
  "co2_per_capita",           # RESPONSE
...
if (any(!is.finite(as.matrix(data_log %>% select(starts_with("log_")))))) {
  ...
  data_log <- data_log %>%
    filter(if_all(starts_with("log_"), is.finite))
...
null_model <- lm(log_co2 ~ 1, data = data_log)
current_r2 <- 0
current_adj_r2 <- 0
...
while (length(remaining_predictors) > 0) {
  ...
  for (pred in remaining_predictors) {
    ...
    formula_test <- as.formula(paste("log_co2 ~", paste(test_predictors, collapse = " + ")))
...
p_r2 <- ggplot(r_squared_history, aes(x = Step)) +
  ...
  labs(
    ...
    subtitle = paste("Final model:", length(selected_predictors), "predictors for CO2 emissions"),
    x = "Step",
    y = "R²",
    color = "Metric"
  ) +
...
if (length(selected_predictors) == 0) {
  ...
  formula_final <- as.formula(paste("log_co2 ~", paste(selected_predictors, collapse = " + ")))
...
cat("Response: log(co2_per_capita + 1)\n")
cat(strrep("=", 80), "\n\n")
...
if (length(selected_predictors) > 1) {
  ...
  p_vif <- ggplot(vif_df, aes(x = reorder(Variable, VIF), y = VIF)) +
    ...
    coord_flip() +
    labs(title = "Variance Inflation Factors (VIF) - CO2 Model",
