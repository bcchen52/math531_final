# =============================================================================
# Advanced OLS Analysis: Coal CO2 Emissions as Response
# =============================================================================
...
#   4. Influential point removal and model refitting
#
# Response: coal_co2_per_capita (log-transformed)
# Predictors: GDP, year, energy, and other non-CO2 variables (selected via stepwise)
# =============================================================================
...
output_dir <- "plots/ols_coal_response"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

cat("\n", strrep("=", 80), "\n")
cat("ADVANCED OLS ANALYSIS: COAL CO2 EMISSIONS PREDICTION\n")
cat("Response: coal_co2_per_capita (log-transformed)\n")
cat(strrep("=", 80), "\n\n")
...
data1 <- data0 %>%
  ...
  drop_na(coal_co2_per_capita)  # Response variable must be non-NA

# Select variables (excluding all co2_per_capita variables except coal)
wanted_columns <- c(
  "iso_code",
  "year",
  "coal_co2_per_capita",      # RESPONSE
  "gdp_per_capita",           # Predictor
  "flaring_co2_per_capita",   # Keep this CO2 source
...
if (any(!is.finite(as.matrix(data_log %>% select(starts_with("log_")))))) {
  ...
  data_log <- data_log %>%
    filter(is.finite(log_coal) & is.finite(log_gdp) & is.finite(log_flaring) &
           is.finite(log_land_use) & is.finite(log_energy) & 
           is.finite(log_methane) & is.finite(log_nitrous))
  cat(sprintf("  - Cleaned data: %d rows (removed non-finite values)\n", nrow(data_log)))
}
...
null_model <- lm(log_coal ~ 1, data = data_log)
current_r2 <- 0
current_adj_r2 <- 0
...
while (length(remaining_predictors) > 0) {
  ...
  for (pred in remaining_predictors) {
    ...
    formula_test <- as.formula(paste("log_coal ~", paste(test_predictors, collapse = " + ")))
...
p_r2 <- ggplot(r_squared_history, aes(x = Step)) +
  geom_line(aes(y = R_squared, color = "R²"), linewidth = 1.2) +
  geom_point(aes(y = R_squared, color = "R²"), size = 3) +
  geom_line(aes(y = Adj_R_squared, color = "Adj R²"), linewidth = 1.2, linetype = "dashed") +
  ...
  labs(
    ...
    subtitle = paste("Final model:", length(selected_predictors), "predictors for coal CO2 emissions"),
    x = "Step",
    y = "R²",
    color = "Metric"
  ) +
...
if (length(selected_predictors) == 0) {
  ...
  formula_final <- as.formula(paste("log_coal ~", paste(selected_predictors, collapse = " + ")))
...
cat("Response: log(coal_co2_per_capita + 1)\n")
cat(strrep("=", 80), "\n\n")
...
if (length(selected_predictors) > 1) {
  ...
  p_vif <- ggplot(vif_df, aes(x = reorder(Variable, VIF), y = VIF)) +
    ...
    coord_flip() +
    labs(title = "Variance Inflation Factors (VIF) - Coal CO2 Model",
